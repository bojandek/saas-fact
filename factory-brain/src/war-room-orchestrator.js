...
class WarRoomOrchestrator {
    ...
    async sendMessage(message) {
        ...
        this.log.info({ sender: msg.sender, recipient: msg.recipient, type: msg.type }, msg.content);
    }
    getMessageLog() {
    ...
    async orchestrateParallel(agents) {
        this.log.info({ agents: agents.map((a) => a.name), count: agents.length }, 'Starting parallel agent execution');
        await this.sendMessage({
            sender: 'Orchestrator',
    ...
    async runFullPipeline(pipelineAgents) {
        ...
        if (failures.length > 0) {
            this.log.warn({ failures: failures.map((f) => ({ name: f.name, error: f.error?.message })) }, 'Some parallel agents failed — continuing to deploy with partial results');
        }
        ...
        try {
            const learningLoop = await (0, autonomous_learning_loop_1.getLearningLoop)();
            const outcome = {
        ...
        try {
            const memory = (0, memory_orchestrator_1.getMemoryOrchestrator)();
...
exports.WarRoomOrchestrator = WarRoomOrchestrator;
//# sourceMappingURL=war-room-orchestrator.js.map