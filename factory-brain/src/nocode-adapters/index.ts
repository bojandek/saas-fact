...
abstract class BaseAdapter {
  ...
  protected parseColumns(sqlSchema: string, tableName: string): Array<{ name: string; type: string }> {
  ...
  protected parseApiEndpoints(apiSpec: string): Array<{ path: string; method: string; description: string }> {
...
export class FlutterFlowAdapter extends BaseAdapter {
  ...
  private mapSqlTypeToFlutterFlow(sqlType: string): string {
  ...
  private generateFlutterFlowPages(blueprint: Blueprint, tables: string[]): object[] {
    const pages = [
    ...
    for (const table of tables.slice(0, 5)) {
      ...
      pages.push(
        { name: `${name}ListPage`, route: `/${table}`, type: 'list' },
        { name: `${name}DetailPage`, route: `/${table}/:id`, type: 'detail' }
      )
    }
...
export class BubbleAdapter extends BaseAdapter {
  ...
  private mapSqlTypeToBubble(sqlType: string): string {
    const typeMap: Record<string, string> = {
      uuid: 'text',
      text: 'text',
      varchar: 'text',
  ...
  private generateBubbleWorkflows(blueprint: Blueprint): object[] {
    return [
      {
        name: 'User signs up',
...
export class RetoolAdapter extends BaseAdapter {
...
export class NoCodeAdapterFactory {
  private adapters: Map<NoCodePlatform, BaseAdapter> = new Map([
  ...
  convertAll(blueprint: Blueprint): Record<NoCodePlatform, AdapterOutput> {
    const result = {} as Record<NoCodePlatform, AdapterOutput>
...
// Singleton instance
export const noCodeAdapterFactory = new NoCodeAdapterFactory()
