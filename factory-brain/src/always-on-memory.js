...
exports.AlwaysOnMemoryEngine = void 0;
...
class AlwaysOnMemoryEngine {
    ...
    async reason(input, context = {}) {
        ...
        try {
            ...
            step.reasoning = await this.generateReasoning(input, matchedPatterns, applicableRules, context);
            ...
            logger_1.logger.error('Reasoning error:', error);
            throw error;
        }
    }
    /**
    ...
    calculatePatternScore(input, pattern, context) {
        ...
        const triggersMatched = pattern.context_triggers.filter(trigger => input.toLowerCase().includes(trigger.toLowerCase())).length;
        score += (triggersMatched / pattern.context_triggers.length) * 0.6;
        // Check user preferences alignment
        const userContextMatch = Object.entries(context).filter(([key, value]) => pattern.context_triggers.some(t => t.toLowerCase().includes(key.toLowerCase()))).length;
     ...
     */
    applyDecisionRules(input, context, patterns) {
        if (!this.memoryState)
            return [];
        const applicable = [];
     ...
     */
    evaluateCondition(condition, input, context, patterns) {
     ...
     */
    async generateReasoning(input, patterns, rules, context) {
        const reasoningParts = [
            `Analyzing: "${input}"`,
            `Context: ${Object.keys(context).join(', ') || 'none'}`,
        ];
        if (patterns.length > 0) {
            reasoningParts.push(`Matched patterns: ${patterns.map(p => p.name).join(', ')}`);
        }
        if (rules.length > 0) {
            reasoningParts.push(`Applying rules: ${rules.map(r => r.action).join('; ')}`);
        }
     ...
     */
    calculateConfidence(patterns, rules) {
     ...
     */
    async recordDecision(decisionId, action, feedback, notes) {
        if (!this.memoryState)
            return;
        // Record feedback
        const feedbackEntry = {
     ...
     */
    async updateRulesBasedOnFeedback(action, feedback) {
        if (!this.memoryState)
            return;
        for (const rule of this.memoryState.decision_rules) {
     ...
     */
    async learnPattern(name, description, triggers, recommendation) {
        if (!this.memoryState)
            return;
        const pattern = {
            name,
     ...
     */
    async updatePatternEffectiveness(patternName, successful) {
        if (!this.memoryState)
            return;
    ...
    async analyzeLearning() {
        ...
        const avgConfidenceTrend = recentSteps.length > 0
        ...
        if (avgConfidenceTrend < 0.4 && this.memoryState.decision_rules.length < 20) {
            ...
            logger_1.logger.info('Low confidence detected - considering new patterns');
        }
        // Clean up low-effectiveness patterns
        this.memoryState.learned_patterns = this.memoryState.learned_patterns.filter(p => p.effectiveness > 0.2 || p.frequency < 3);
        // Verify rules performance
        const poorPerformingRules = this.memoryState.decision_rules.filter(r => r.success_rate < 0.3 && r.conditions_met > 5);
        if (poorPerformingRules.length > 0) {
            logger_1.logger.info(`Found ${poorPerformingRules.length} underperforming rules`);
            // Mark for review
        }
        await this.saveMemoryState();
    }
    ...
    async saveMemoryState() {
        ...
        if (error)
            logger_1.logger.error('Failed to save memory state:', error);
    }
    /**
     * Get memory insights
     */
    async getInsights() {
        ...
        const avgConfidence = this.reasoningChain.length > 0
...
exports.AlwaysOnMemoryEngine = AlwaysOnMemoryEngine;
//# sourceMappingURL=always-on-memory.js.map