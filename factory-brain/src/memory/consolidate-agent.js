...
exports.startConsolidationCron = startConsolidationCron;
exports.getConsolidationHistory = getConsolidationHistory;
const supabase_js_1 = require("@supabase/supabase-js");
const client_1 = require("../llm/client");
const embeddings_1 = require("../llm/embeddings");
const logger_1 = require("../utils/logger");
const retry_1 = require("../utils/retry");
const log = logger_1.logger.child({ agent: 'ConsolidateAgent' });
// ── Clients ───────────────────────────────────────────────────────────────────
...
// ── Database Operations ───────────────────────────────────────────────────────
async function readUnconsolidatedMemories(limit = 10) {
    ...
    const { data, error } = await supabase
        ...
        .limit(limit);
    if (error)
        throw new Error(`Failed to read unconsolidated memories: ${error.message}`);
    return (data ?? []);
}
async function storeConsolidation(input) {
    const supabase = getSupabase();
    // Generate embedding for the insight
    const embeddingText = `${input.summary} ${input.insight}`;
    const embeddingResponse = await (0, retry_1.withRetry)(() => (0, embeddings_1.createEmbedding)({
    ...
    if (insertError)
        throw new Error(`Failed to store consolidation: ${insertError.message}`);
    ...
    for (const conn of input.connections) {
        for (const memId of [conn.from_id, conn.to_id]) {
            const { data: memData } = await supabase
                ...
                .single();
            if (memData) {
                const existing = memData.connections ?? [];
    ...
    if (updateError)
        throw new Error(`Failed to mark memories as consolidated: ${updateError.message}`);
    log.info({ memories_processed: input.source_ids.length, insight: input.insight.slice(0, 80) }, 'Consolidation stored');
    return {
        status: 'consolidated',
...
Return valid JSON only, no markdown.`;
    ...
    return {
        ...
        connections: Array.isArray(parsed.connections)
            ? parsed.connections.filter((c) => typeof c.from_id === 'number' &&
 ...
 */
async function runConsolidationCycle(minMemories = 2, batchSize = 10) {
    ...
    log.info({ insight: result.insight.slice(0, 100), memories_processed: result.memories_processed }, 'Consolidation cycle complete');
    return result;
}
/**
 ...
 */
function startConsolidationCron(intervalMs = 60 * 60 * 1000, // 1 hour default
minMemories = 2) {
 ...
 */
async function getConsolidationHistory(limit = 10) {
    const supabase = getSupabase();
    const { data, error } = await supabase
        .from('memory_consolidations')
        ...
        .limit(limit);
    if (error)
        throw new Error(`Failed to get consolidation history: ${error.message}`);
    return data ?? [];
}
//# sourceMappingURL=consolidate-agent.js.map