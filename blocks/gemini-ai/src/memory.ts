/**
 * Memory system for Factory Brain with Gemini
...
export class MemorySystem {
  ...
  async recordPattern(pattern: Pattern): Promise<void> {
    const { error } = await this.supabase
      ...
      .insert({
        ...
        frequency_used: pattern.frequency_used,
      })
   ...
   */
  async getTopPatterns(limit: number = 5): Promise<Pattern[]> {
   ...
   */
  async getLessonsByCategory(category: Lesson['category']): Promise<Lesson[]> {
    const { data, error } = await this.supabase
      .from('lessons')
      .select('*')
