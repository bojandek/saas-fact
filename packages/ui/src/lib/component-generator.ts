...
export class NanoBananaComponentGenerator {
  /**
...
interface ${componentName}Props {
  ${properties?.fields?.map((f: any) => `${f.name}?: string;`).join('\n  ')}
  onSubmit?: (data: any) => void;
}

export const ${componentName}: React.FC<${componentName}Props> = ({
  ${properties?.fields?.map((f: any) => f.name).join(', ')}
  onSubmit,
}) => {
...
.${componentName} button:hover {
  opacity: 0.9;
    ...
    return {
      ...
      code: code.trim(),
      styles: styles.trim(),
      props: properties?.fields?.reduce((acc: any, f: any) => {
...
export const ${componentName}: React.FC<${componentName}Props> = ({
   ...
   */
  static generateComponentLibrary(theme: ComponentGeneratorConfig['theme'], components: Array<{ name: string; type: string }>): GeneratedComponent[] {
    return components.map(comp => {
      const config: ComponentGeneratorConfig = {
